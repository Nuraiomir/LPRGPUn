...
def main():
        ...
        while True:
            ...
            if aspect > SQUARE_ASPECT_MAX or (
                ...
                if square_ocr_frames % OCR_EVERY_N_DETECTIONS == 1:
                    ...
                    # Keep square OCR as the primary interpretation.
                    submit_ocr("square", sq, t, det)

                    # After the square vehicle is confirmed, also sample the
                    # same detected crop with normal OCR. This catches the next
                    # horizontal plate even if YOLO's bbox still looks square
                    # for a few frames during the transition.
                    if confirmed_plate == "633BBT02":
                        if 9.0 <= t <= 13.5:
                            print(
    ...
    result = {
        "version": "v9_true_async_gpu_strong_switch_694BPT05",
        "video": str(VIDEO),
        "fps": fps,
